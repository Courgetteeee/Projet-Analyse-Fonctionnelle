...
-   **Ordre** : l'intervalle temporel [1,672] est ordonnée et muni d'une distance : l'instant $t_10$ arrive necessairement après $t_9$, ce qui distinque les données fonctionnelles d'un simple vecteur de variables.
...
Nous calculons donc le $\delta^*$ optimal pour chaque individu séparément en balayant une grille de valeurs. Une approche individuelle est plus rigoureuse qu'un $\delta$ commun car les entreprises peuvent avoir des profils de consommation très différents.
...
ggplot( df_plot_test1, aes(x=log10(delta), y=gcv)) +
  ...
  labs( title = paste("Selection de delta par GCV pour l'individu", i), x="Paramètre de lissage en log", y="GCV(delta)")
```
...
La méthode décrite ici provient du livre $\underline{Functional\ Data\ Analysis\ with\ R}$. Le modèle est identique à l'Alternative 1. La seule différence réside dans la sélection du paramètre de lissage $\delta$ : au lieu d'une recherche manuelle par GCV, celui-ci est estimé automatiquement par la méthode REML (Restricted Maximum Likelihood) intégrée à gam(). Cette approche est plus rapide et évite de définir une grille de valeurs.

On utilise la fonction $gam()$ pour ajuster les courbes lisses en spécifiant un terme de lissage par splines cubiques (bs = "cr"). Le paramètre k=40 correspond à la dimension de la base q=40, cohérent avec la règle de Ruppert appliquée dans l'Alternative 1.
...
Approche, proposée par Xiao et al. (2016) (à développer commentaire Goga)
